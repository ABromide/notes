# (4 封私信 / 2 条消息) 记录一次业务场景下的 Agent SFT 与 RL 训练实践 - 知乎
今天是五一假期第二天，照也拍了，视频也刷够了，干脆把去年做的事情整理出来分享一下。一方面是给自己做个记录，免得时间久了细节都忘干净了；另一方面，希望对同样在做 Agent 训练的朋友有些参考价值。

这次 Agent 训练的主要工作从 25 年 8 月持续到 25 年 12 月，完整覆盖了数据构建、[SFT 监督微调](https://zhida.zhihu.com/search?content_id=274150433&content_type=Article&match_order=1&q=SFT+%E7%9B%91%E7%9D%A3%E5%BE%AE%E8%B0%83&zhida_source=entity)和 RL 强化学习训练三个阶段。此后的时间则主要集中在数据质量优化和奖励模型的迭代改进上。
...
25 年 8 月时，可参考的 Agent 方案其实并不多，当时视野内有：阿里系的 Deep Research、Kimi 的 Researcher、问小白的 Deep Research Bench，以及 OPPO 的 [COA](https://zhida.zhihu.com/search?content_id=274150433&content_type=Article&match_order=1&q=COA&zhida_source=entity)（Chain of Agents，链式智能体框架）。
...
COA：是oppo发布的一篇论文[https://chain-of-agents-afm.github.io/](https://link.zhihu.com/?target=https%3A//chain-of-agents-afm.github.io/)，在这篇论文里面，提出了“链式智能体”（Chain of Agents COA）框架，用多智能体蒸馏+智能体强化学习，一次性训练出端到端的智能体基础模型（AFM）。 通过轨迹构建、SFT、RL三段式流程，首次实现了端到端的多Agent协作推理模式，在web、code、math等领域取得突破性性能。

![](https://pic2.zhimg.com/v2-3ddd745cb21be89fc44cd14ccf1d86a9_1440w.jpg)
...
3.  `output`：模型按以下固定流程循环推理，直至得出答案： `think` → `plan` → `think` → `wiki_search` → `observation` → `think` → `reflection` → `think` → `wiki_search` → `observation` → `think` → `answer`
...
至此，数据格式基本确定。接下来的重点转向两个工程环节：SFT 训练框架的适配，以及 [vLLM 推理](https://zhida.zhihu.com/search?content_id=274150433&content_type=Article&match_order=1&q=vLLM+%E6%8E%A8%E7%90%86&zhida_source=entity)侧的调用逻辑改造。
...
![](https://pic4.zhimg.com/v2-fec3ecf4f203d3c6e86baff6a592d3e1_1440w.jpg)

* * *

### [GRPO 训练](https://zhida.zhihu.com/search?content_id=274150433&content_type=Article&match_order=1&q=GRPO+%E8%AE%AD%E7%BB%83&zhida_source=entity)
...
![](https://pic2.zhimg.com/v2-d853a28d0a3b4416895aecb1b91500e3_1440w.jpg)
...
![](https://pic2.zhimg.com/v2-6c077951081dc294898e704b446deb05_1440w.jpg)

search-r1 结构

![](https://pic1.zhimg.com/v2-c118d1226b03e0fe06c449b5ecf11472_1440w.jpg)

自定义结构插入的位置

**2\. 为 rollout 增加 stop\_words 支持**
...
核心部分的代码（`LLMGenerationManager`）已上传至：[llm\_agent/business\_agent.py](https://link.zhihu.com/?target=https%3A//github.com/yuanzhoulvpi2017/nano_rl/blob/main/llm_agent/business_agent.py)
