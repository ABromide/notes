...
> 作者：storm  
> 原文：https://zhuanlan.zhihu.com/p/2025536259628569667

英文博客:http://yuqianfu.notion.site/revisiting-opd  
论文链接:https://huggingface.co/papers/2603.25562  
开源代码:https://github.com/hhh675597/revisiting\_opd

作者：Yuqian Fu @storm, Haohuan Huang @iBanana1, Kaiwen Jiang, Jiacai Liu @skydownacai, Zhuo Jiang @炼熵师, Yuanheng Zhu†, Dongbin Zhao

![](https://pic4.zhimg.com/v2-26121b06d91ba51ba72c4868549c700b_1440w.jpg)

在最近的大模型后训练中，On-Policy Distillation (OPD) 已经成了默认选项之一。但我们@storm @iBanana1 @skydownacai @炼熵师 近期在分析训练日志、复现实验和对比不同 OPD 实现时，反复碰到同一个问题：理论上很自然的 sampled-token OPD，实际跑起来并不稳定，甚至会把模型往一些局部上“看起来合理”、整体上却越来越差的方向推。

在这篇文章中，我们并不打算再次讲解 OPD (已经有很多不错的入门材料)，而是想集中回答三个更具体的问题：**这个方法到底在优化什么；常见实现为什么容易出问题；以及有没有一个代价不高、但更稳定的实现路径。** 

On-policy distillation（OPD）是在学生模型自己生成的轨迹上，用教师模型的反馈来训练学生模型的方法 \[1\]\[2\]\[3\]。而**在长时程大模型后训练里，常见的 sampled-token 形式 往往并不稳定**。
...
更详细的推导可见[英文版](https://yuqianfu.notion.site/revisiting-opd)附录 A。
...
![](https://picx.zhimg.com/v2-4e74e04ed8dba6dfc60dd983cad15aa3_1440w.jpg)
...
![](https://picx.zhimg.com/v2-efbee5cd0e9fc4e7924efd2062cae395_1440w.jpg)
...
这里推荐一下两位老师 Jiacai Liu @skydownacai, Zhuo Jiang @炼熵师 的推导，可用于延伸阅读：[The Policy Gradient, Bias, and Variance of OPD](https://yuqianfu.notion.site/policy-gradient-bias-and-variance-of-opd)
...
![](https://pic4.zhimg.com/v2-ed4e1e258217ef6785de84012b12464b_1440w.jpg)
...
![](https://pic2.zhimg.com/v2-096916963db3e9bc8143cfa7eb0a3cf3_1440w.jpg)

![](https://pic1.zhimg.com/v2-733f12d112fb61490c5eed010f8f9078_1440w.jpg)
...
于是你会得到一个很大的负奖励，尽管两个模型在语义上其实想表达的是同一件事。类似的问题也会出现在 special-token 上，例如 end-of-sequence（EOS） 标记（如 <|im\_end|> 与 <endoftext>），它们语义等价，但概率比较出来却可能完全错位。

![](https://pica.zhimg.com/v2-ef19c17e9dc1375465ff7c9e9a3541d2_1440w.jpg)
...
这也是为什么局部信号会更平衡：正向和负向的调整会分布在一个局部决策区域里，而不是集中打在单个 token 上。我们发现在 [verl 的 Megatron-OPD recipe](https://github.com/verl-project/verl-recipe/blob/main/gkd/megatron/megatron_distill_losses.py)中也实现了这种做法。
...
| Agentic | Math | Math | Math | Math | Math | Math |
| --- | --- | --- | --- | --- | --- | --- |
| Method | ALFWorld | MATH500 | AIME24 | AIME25 | Minerva | OlympiadBench |
| Qwen2.5-7B-It | 21.9 | 68.2 | 13.3 | 0.0 | 26.5 | 32.9 |
| GiGPO-Qwen2.5-7B-It-Alfworld | 95.3 | \- | \- | \- | \- | \- |
| OpenThinker3-7B | \- | 92.2 | 53.3 | 40.0 | 39.0 | 55.6 |
| Sampled-token OPD | 90.6 | 74.8 | 13.3 | 13.3 | 32.1 | 40.5 |
| Sampled-token OPD w/ mask | 93.8 | 76.0 | 20.0 | 13.3 | 33.5 | 40.4 |
| Ours w/o mask | 95.3 | 82.0 | 33.3 | 16.7 | 32.7 | 44.0 |
| Ours w/ mask | 97.7 | 79.0 | 20.0 | 16.7 | 34.6 | 42.5 |
...
![](https://pic1.zhimg.com/v2-4d106ac09f750265959691d229352974_1440w.jpg)

![](https://pic4.zhimg.com/v2-e43ee4e2235d8bbc4fcd13921d5d0671_1440w.jpg)
...
![](https://pic1.zhimg.com/v2-0eec5a52164993a3c2c32f6d7af0c49e_1440w.jpg)

![](https://pic1.zhimg.com/v2-b5254549e6a628eb0a0f7100ebd8849a_1440w.jpg)

图 7. 策略更新过程中的优化统计量。

图 7 比较了我们的方法和 sampled-token OPD 基线在优化统计量上的差异。teacher top-K 目标在保持足够 entropy 的同时，呈现出更小的 gradient norm。这个现象在单任务和多任务设置下都能看到。

![](https://picx.zhimg.com/v2-293104635fdeae09a737dd8de79fd7c9_1440w.jpg)
...
![](https://pica.zhimg.com/v2-ca0e441bda046693be144e1fe1cc326e_1440w.jpg)

![](https://pic4.zhimg.com/v2-71529c54294c294de741be51dba9c603_1440w.jpg)
...
| avg@32 | pass@1 | pass@1 | pass@1 | pass@1 | pass@1 | pass@1 |
| --- | --- | --- | --- | --- | --- | --- |
| Method | AIME24 | MATH500 | AIME24 | AIME25 | Minerva | OlympiadBench |
| teacher top-K | 23.6 | 80.4 | 23.3 | 26.7 | 34.2 | 43.9 |
| student top-K | 22.3 | 82.4 | 30.0 | 16.7 | 35.7 | 44.9 |
| teacher top-K w/ sampled token | 22.4 | 81.6 | 26.7 | 23.3 | 36.4 | 46.7 |
...
| teacher top-K | 95.3 | 82.0 | 33.3 | 16.7 | 32.7 | 44.0 |
| student top-K | 95.3 | 65.6 | 10.0 | 10.0 | 25.0 | 31.6 |
| teacher top-K w/ sampled token | 94.5 | 63.2 | 10.0 | 10.0 | 21.0 | 30.1 |
